...
screen friend_screen(controller, friends, rect):
    ...
    # display screens
    use friend_info(controller, friends, info_rect)
    use friend_description(controller, description_rect)

screen friend_info(controller, friends, rect):
    vbox:
        xoffset rect.left
        yoffset rect.top
        xsize rect.width
            ...
            side "c r":
                ...
                vpgrid id(_vpgrid_name):
                    ...
                    for friend in friends:
                        use friend_entry(controller, friend, rect)
                vbar value YScrollValue(_vpgrid_name)

# Show friend info
screen friend_entry(controller, friend, rect):
    default iconSize = 80
    frame:
        style "friend_info_entry"
        xsize rect.width-25
        background Solid(controller.get_friend_colour(friend))
        hbox:
            xsize rect.width
            ysize iconSize
            spacing 5
...
screen friend_description(controller, rect):
