...
screen bag_screen(controller, inventory, rect):
    use bag_inventory(controller, inventory, rect)

# display the bag inventory
screen bag_inventory(controller, inventory, rect):
    # render inventory
    $ border_size = 5
        ...
        hbox:
            ...
            vpgrid id (_grid_name):
                ...
                for item in inventory.items:
                    use bag_item(controller, item, item_rect.width, item_rect.height)
...
# display a bag item
screen bag_item(controller, item, width, height):
    frame:
        xsize width
        ysize height
        background Solid(controller.get_item_colour(item))
        imagebutton:
            xmaximum width
            ymaximum height
            ...
            hovered [Show("bag_tooltip", None, controller, item)]
            unhovered [Hide("bag_tooltip")]

# hovering tooltip for each item when hovered
screen bag_tooltip(controller, item, rect=Rect2D(left=300, right=600, top=200, bottom=400)):
    if item:
        ...
        vbox:
            ...
            frame:
                background Solid(controller.get_tooltip_background_colour())
                has vbox
                text "{b}Name: {/b}" + "{0}".format(item.name)
                frame:
                    xminimum rect.left
                    xmaximum rect.right
                    background Solid(controller.get_tooltip_colour(item))
                    vbox:
